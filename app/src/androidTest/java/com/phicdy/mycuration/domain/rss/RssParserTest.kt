...
class RssParserTest {
    ...
    fun parseRSSVersion1AndFirstSizeIsCorrect() {
        val articles = parser.parseArticlesFromRss(RssV1().text().byteInputStream(), -1)
    ...
    fun parserRSSVersion2AndSizeIsCorrect() {
        val articles = parser.parseArticlesFromRss(RssV2().text().byteInputStream(), -1)
        assertThat(articles.size).isEqualTo(2)
    }
    ...
    @Test
    fun parserAtom() {
        val articles = parser.parseArticlesFromRss(Atom().text().byteInputStream(), -1)
    ...
    fun parserAtomAndroidDeveloperBlog() {
        val articles = parser.parseArticlesFromRss(AtomAndroidDeveloperBlog().text.byteInputStream(), -1)
        assertThat(articles[0].url)
